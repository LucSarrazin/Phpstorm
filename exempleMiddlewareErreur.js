...
app.get('/eleveserreur', (requete, reponse) => {
    let erreur = new Error(`Traitement d'une erreur dans la requete a l'adresse ${requete.url}`)
...
app.get('/eleves', async (requete, reponse) => {
    try{
        const apiResponse = await axios.get("http://localhost:3001/api/leseleves")
        const jsonResponse = apiResponse.data

        reponse.send(jsonResponse)
    }catch(erreur){
        next(erreur) // On appelle le middleware de gestion des erreurs suivant
    }
